...
REFRESH_LOCK_MAX_AGE_SECONDS = 30 * 60
...
def get_ad_accounts():
    ...
    sources = [
        ...
        ("business/owned_ad_accounts", f"{BASE_URL}/{API_VERSION}/{BUSINESS_ID}/owned_ad_accounts"),
        ("business/client_ad_accounts", f"{BASE_URL}/{API_VERSION}/{BUSINESS_ID}/client_ad_accounts"),
    ]
...
def detect_business_unit(account_name="", campaign_name=""):
    text = f"{account_name} {campaign_name}".upper()
    if "US-FB-HR-" in text or "US-BO-HR-" in text or "-FB-HR" in text or "-FB-BO" in text:
        return "VAL"
    return "El - Okaby"
...
def is_refresh_locked():
    ...
    try:
        ...
        if lock_age > REFRESH_LOCK_MAX_AGE_SECONDS:
...
if refresh_clicked:
    if is_refresh_locked():
        st.warning("Refresh is already running. Please wait a few minutes and try again.")
        st.stop()
